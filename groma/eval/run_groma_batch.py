...
def draw_box(box, image, image1,index, output_dir,query,image_name,output_path):
    w, h = image.size
    W, H = image1.size
    ...
    record = {
        ...
        "question": query,
        "result": [
            [box_original[0], box_original[1]],
            [box_original[2], box_original[3]]
        ]
    }

    # 读原 json（如果文件不存在，创建空列表）
    try:
...
def eval_model(model_name, quant_type,anno_path,img_path,output_path):
    # Model
    disable_torch_init()
    ...
    for item in tqdm(images):
        ...
        image_file = os.path.join(image_base_path, image_name)  # 路径 + 文件名
        query = item['caption']
        ...
        raw_image1 = load_image(image_file)
        raw_image = raw_image1.resize((448, 448))
        ...
        for i, box in enumerate(pred_boxes[selected_box_inds, :]):
            ...
            draw_box(box, img_copy,raw_image1, selected_box_inds[i], output_dir,query,image_file.split('/')[-1],output_path)
...
if __name__ == "__main__":
    ...
    eval_model(model_name, args.quant_type, args.anno_path,args.img_path,args.output_path)
